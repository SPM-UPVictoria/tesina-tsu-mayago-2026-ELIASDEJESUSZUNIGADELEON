...
\paragraph{Información General sobre la UAT}
...
\paragraph{Misión Institucional}
...
\paragraph{Visión Institucional}
...
\paragraph{El Portal Institucional: Situación Actual}
...
\paragraph{El Proyecto de Modernización}
...
\paragraph{Migración de SharePoint: De Versiones Locales a Cloud}

Microsoft SharePoint es una plataforma de gestión de contenidos ampliamente utilizada en instituciones educativas y gubernamentales \cite{microsoft2024sharepoint}. Existen dos modelos operacionales: SharePoint Server (versiones 2013, 2016, 2019) instaladas localmente, y SharePoint Online, la versión moderna basada en cloud integrada a Office 365/Microsoft 365.

Las versiones locales presentan limitaciones importantes:
\begin{itemize}
    \item Requieren infraestructura propia (servidores, mantenimiento, actualización).
    \item No reciben actualizaciones de seguridad continuas.
    \item Tienen funcionalidades limitadas comparadas con versiones modernas.
    \item Implican costos operacionales significativos.
\end{itemize}

SharePoint Online ofrece ventajas estratégicas \cite{microsoft2024online}:
\begin{itemize}
    \item Actualizaciones de seguridad y funcionalidades automáticas.
    \item Escalabilidad flexible sin inversión en infraestructura.
    \item Integración con herramientas de Microsoft 365 (Teams, Power BI, etc.).
    \item Mejor rendimiento global y disponibilidad con SLA garantizado.
    \item Cumplimiento automático de normativas internacionales.
\end{itemize}

\paragraph{Diseño Web Responsivo y Accesibilidad Digital}

El diseño responsivo garantiza que un sitio web se adapte correctamente a diferentes tamaños de pantalla (computadoras, tablets, dispositivos móviles) \cite{marcotte2020responsive}. La accesibilidad digital se refiere a la capacidad de todos los usuarios, incluyendo personas con discapacidades, de acceder y usar contenido web efectivamente.

Las normativas internacionales más relevantes son \cite{w3c2021wcag}:
\begin{itemize}
    \item \textbf{WCAG 2.1 (Web Content Accessibility Guidelines):} Estándar del W3C que define criterios de accesibilidad en tres niveles (A, AA, AAA).
    \item \textbf{Directiva de Accesibilidad Digital (UE):} Obliga a instituciones públicas a cumplir WCAG 2.1 nivel AA mínimo.
\end{itemize}

Un portal institucional moderno debe cumplir con estos estándares para garantizar inclusión y acceso equitativo a toda su comunidad \cite{henry2016just}.

\paragraph{Frontend Moderno: React y Bootstrap}

Los frameworks frontend modernos permiten construir interfaces de usuario interactivas y responsivas de manera eficiente \cite{react2024facebook}. React es una librería JavaScript que facilita componentes reutilizables, gestión eficiente del estado, y renderizado dinámico optimizado.

Bootstrap es un framework CSS que proporciona \cite{bootstrap2024}:
\begin{itemize}
    \item Sistema de grillas responsivo.
    \item Componentes preconstruidos (botones, formularios, navegación).
    \item Utilidades CSS para rapidez de desarrollo.
    \item Cumplimiento de estándares de accesibilidad por defecto.
\end{itemize}

La combinación de React + Bootstrap permite desarrollar interfaces modernas, accesibles y mantenibles en tiempo corto.

\paragraph{Optimización de Arquitectura de Datos}

La arquitectura de bases de datos impacta directamente en el rendimiento del portal \cite{silberschatz2020database}. Las limitaciones comunes incluyen redundancia de datos, consultas ineficientes, estructura no normalizada y falta de índices.

La optimización incluye \cite{codd1990relational}:
\begin{itemize}
    \item \textbf{Normalización:} Aplicación rigurosa de formas normales.
    \item \textbf{Consolidación de tablas:} Eliminación de redundancias.
    \item \textbf{Indexación estratégica:} Creación de índices en campos frecuentemente consultados.
    \item \textbf{Caché de consultas:} Reducción de consultas repetitivas.
\end{itemize}

\paragraph{Integración de APIs Externas}

Los portales modernos se integran con servicios externos mediante APIs \cite{fielding2000rest}. Ejemplos relevantes incluyen mapas interactivos (OpenStreetMap, Google Maps), redes sociales (Instagram, YouTube), servicios de email, autenticación (OAuth, SAML) y análisis web \cite{openstreetmap2024contributors}.
...
\paragraph{Sobre la Migración Tecnológica}
\begin{itemize}
...
\paragraph{Sobre la Optimización de Datos}
\begin{itemize}
...
\paragraph{Sobre el Rediseño de Secciones}
\begin{itemize}
...
\paragraph{Sobre la Integración de Herramientas}
\begin{itemize}
...
\paragraph{Limitaciones Técnicas}
\begin{itemize}
...
\paragraph{Limitaciones de Alcance}
\begin{itemize}
...
\paragraph{Limitaciones Temporales}
\begin{itemize}
...
\paragraph{Limitaciones de Recursos}
\begin{itemize}
